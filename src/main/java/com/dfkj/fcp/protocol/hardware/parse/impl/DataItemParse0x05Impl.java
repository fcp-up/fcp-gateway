 ...
 * @date 2016-05-26
 */
@DataItemParseAnnotation(dataTypeId = 0x05)
public class DataItemParse0x05Impl implements IDataItemParseGtw1P1 {

	public DataMessageItem[] parse(ByteArray content,int channelNo, int positionNo) {
		DataMessageItem item = new DataMessageItem();
		
		short airHum = content.getShortAt(0);
		content.removeAt(0, 2);
		
		item.setValue(airHum / 10.0);
		item.setDevType(EDeviceType.AIR_HUMIDITY);
